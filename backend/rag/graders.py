...
def retrieval_grader(model: str="command-r-plus-08-2024", temperature: float=0):
    ...
    doc_grader_llm = ChatCohere(model=model, temperature=temperature).with_structured_output(GradeDocuments, preamble=preamble)
...
def hallucination_grader(model: str="command-r-plus-08-2024", temperature: float=0):
    ...
    hallucination_grader_llm = ChatCohere(model=model, temperature=temperature).with_structured_output(GradeHallucination, preamble=preamble)
...
def answer_grader(model: str="command-r-plus-08-2024", temperature: float=0):
    ...
    answer_grader_llm = ChatCohere(model=model, temperature=temperature).with_structured_output(AnswerGrader, preamble=preamble)
